#ifndef __UNIFIED_OS_IO_CPU_CPU_H
#define __UNIFIED_OS_IO_CPU_CPU_H
...
namespace UnifiedOS{
    ...
    namespace CPU{
        struct CPU {
            ...
            void* gdt;   // GDT
            GlobalDescriptorTable::GDTDescriptor gdtPtr;
            Processes::Process* currentProcess = nullptr;
            Processes::Process* idleProcess;
            volatile int QueueLock = 0;
            Vector<Processes::Process*>* Queue;
            GlobalDescriptorTable::TSS tss __attribute__((aligned(16)));
        };
        ...
        static inline  void SetCPULocal(CPU* val) {
            val->self = val;
            asm volatile("wrmsr" ::"a"((uint64_t)val & 0xFFFFFFFF) /*Value low*/,
                        "d"(((uint64_t)val >> 32) & 0xFFFFFFFF) /*Value high*/, "c"(0xC0000102) /*Set Kernel GS Base*/);
            asm volatile("wrmsr" ::"a"((uint64_t)val & 0xFFFFFFFF) /*Value low*/,
                        "d"(((uint64_t)val >> 32) & 0xFFFFFFFF) /*Value high*/, "c"(0xC0000101) /*Set Kernel GS Base*/);
        }
